...
class model_transaksi extends CI_Model {
  ...
  {
      $query = $this->db->query("SELECT a.id, a.id_pembeli, b.nama, b.alamat, a.tanggal, a.colly, a.kode, a.bersih, c.harga_bersih, a.transferan, a.saldo FROM transaksi_pembeli a LEFT JOIN harga_cabai_pembeli c ON a.kode=c.kode_cabai AND a.tanggal=c.tanggal INNER JOIN tb_pembeli b ON a.id_pembeli=b.id ORDER BY a.id DESC");
      if($query->num_rows() > 0)
        {
          return $query->result();
        }
  ...
  function harga_today($date){
   ...
   function harga_todayPembeli($date){
    $query = $this->db->query("SELECT a.kode, a.jenis, b.harga_bs, b.harga_bersih FROM tb_cabai a JOIN harga_cabai_pembeli b WHERE a.kode=b.kode_cabai AND b.tanggal='$date' ORDER BY a.jenis ASC");
      if($query->num_rows() > 0)
        {
          return $query->result();
        }
